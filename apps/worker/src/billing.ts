...
export const computeBill = (period: BillingPeriod, intervals: IntervalLike[], tariffs: TariffShape[], tz: string, now: Date): BillResult => {
  ...
  return {
    ...
    totalCents: lines.energyPkCents + lines.energyMdCents + lines.energyOpCents + lines.demandCents + lines.fixedCents - lines.exportCreditCents,
...
export const refreshBill = async (site: SiteDoc, at: Date, now = new Date()): Promise<BillDoc> => {
  ...
  const bill = computeBill(period, intervals, tariffs, site.tz, now);
