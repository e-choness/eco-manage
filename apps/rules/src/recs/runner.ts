...
import { OPEN_RECOMMENDATION_STATUSES, quarterOf, siteEventsChannel, type DemandNow, type SiteEvent } from '@ecomanage/shared';
...
//   result = rule.evaluate(inputs) → null | Proposal
...
export const proposeForSite = async (siteId: string, at: Date, deps: ProposeDeps): Promise<RecommendationDoc[]> => {
  ...
  for (const rule of enabled) {
    ...
    let proposal;
    try {
      proposal = rule.evaluate(ctx, params);
    } catch (err) {
      deps.logger.error({ siteId, ruleId: rule.id, err: (err as Error).message }, 'rule failed');
      continue;
    }
    if (!proposal) continue;
    const expiresAt = new Date(proposal.window.start.getTime() - config.approval.expireMin * 60_000);
